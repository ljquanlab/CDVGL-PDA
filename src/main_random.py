...
def main(): # benchmarks with positive-to-negative ratios of 1:1, 1:5 and 1:10
    args = parse_args()
    ...
    full_dataset = np.loadtxt(f"{d_file}/different_radio/{prefix}.txt", delimiter="\t", dtype=int)
    PD_train, temp = train_test_split(full_dataset, test_size=0.2,  random_state=args.seed, stratify=full_dataset[:, 2]) 
    PD_valid, PD_test = train_test_split(temp, test_size=0.5,  random_state=args.seed, stratify=temp[:, 2])
