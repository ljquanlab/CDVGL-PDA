...
import pandas as pd
import os
from utils import PDA_Predictor
import numpy as np


def load_mapping_tables():
    # Load phosphorylation site mapping table
    site_file = "../data/psite.csv"
    disease_file = "../data/Disease.csv"
    
    site_df = pd.read_csv(site_file)
    disease_df = pd.read_csv(disease_file)
    
    # Create mapping dictionaries
    site_to_idx = dict(zip(site_df['Psite'], site_df['psite_id']))
    disease_to_idx = dict(zip(disease_df['Mesh_name'], disease_df['disease_id']))
...
def main():
    ...
    # Load model
    model_path = "../saved_models/1_psite_level_best_model.pth"
    ...
    while True:
        ...
        if choice == '1':
            try:
                print(f"\nAvailable phosphorylation site examples: P52734_S205 ...")
                print(f"Available disease examples: Aarskog Syndrome ...")
                
                site_name = input("Please enter phosphorylation site name (e.g., P52734_S205): ").strip()
                disease_name = input("Please enter disease name (e.g., Aarskog Syndrome): ").strip()
            ...
            file_path = input("Please enter CSV file path ( ../data/example/test_file.csv): ").strip()
            ...
            for i, row in site_df.head(10).iterrows():
                print(f"  {row['Psite']} -> Index: {row['psite_id']}")
            
            print(f"\nTotal diseases: {len(disease_df)}")
            print("\nFirst 10 diseases:")
            for i, row in disease_df.head(10).iterrows():
                print(f"  {row['Mesh_name']} -> Index: {row['disease_id']}")
            
            print(f"\nComplete lists are saved in CSV files:")
            
            print(f"  Phosphorylation sites: ../data/psite.csv")
            print(f"  Diseases: ../data/Disease.csv")
            print("="*50)
