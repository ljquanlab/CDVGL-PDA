...
class LinkPredictor(nn.Module):
    def __init__(self, emb_dim, model = "complex", margin=1.0):
        """
        emb_dim: 512
        model: "complex"
        """
...
class CDVGL(nn.Module):
    def __init__(self, graph,node_dict,edge_dict, in_size, config):
        ...
        self.sematic_attention = Attention(hidden_dim=config.n_hid, attn_drop= config.atten_drop) 
    ...
    def forward(self, graph, data, features):
        ...
        site_blossum = features # [batch,71,128]
        ...
        for i in [node1,node2]:
            node_fused[i] = self.sematic_attention([RGAT_feat[i],HGT_feat[i]])
