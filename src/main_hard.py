...
def train(model, graph, PD_train, PD_valid, PD_test, device, features, args, prefix):
    ...
    model_save_path = f"{model_dir}/{prefix}.pth"

    # =====================================================
    # ① If the optimal model already exists, load it directly and skip training
    # =====================================================
    ...
    with torch.no_grad():
        *_, test_probs, test_labels = model(graph, PD_test, features)
        test_auc, test_aupr, acc, precision, recall, f1, mcc = compute_metrics(
            test_probs, test_labels
        )

        with open("test.txt", "a") as f:
            f.write(
                f"data: {prefix} | ACC: {acc:.4f} | Precision: {precision:.4f} | "
                f"Recall: {recall:.4f} | F1: {f1:.4f} | MCC: {mcc:.4f} | "
                f"AUC: {test_auc:.4f} | AUPR: {test_aupr:.4f} | way: mine\n"
            )
...
def main_hard(): 
    ...
    args = parse_args()
    prefix = args.generalization_benchmarks
    ...
    PD_train = np.loadtxt(f"{d_file}/{prefix}/train_1.txt", delimiter='\t', dtype=int)
    PD_test = np.loadtxt(f"{d_file}/{prefix}/test_1.txt", delimiter='\t', dtype=int)
...
if __name__ == '__main__':
    main_hard() # protein_split, psite_split,pair_split
